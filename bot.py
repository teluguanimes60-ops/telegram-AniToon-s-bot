...
async def cb(_, q):
    ...
    # ---------------- INSTANT EDIT (FIXED) ----------------
        ...
        m = await q.message.reply_text(
            "⚡ Instant Edit Mode Activated\n\n📂 Send file first\n✏️ Then I will ask new name instantly",
            reply_markup=back_btn()
        )
        await clean_old(uid, m)
    ...
    if state["mode"] == "convert":
        ...
        m = await msg.reply(
            ...
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("📹 Video MP4", callback_data=f"conv_video_{job_id}"),
                    InlineKeyboardButton("📁 Keep File", callback_data=f"conv_file_{job_id}")
                ]
            ])
        )
...
async def text_handler(_, msg):
    ...
    # ---------------- INSTANT EDIT (FIXED) ----------------
    if state["mode"] == "instant" and state.get("step") == "name":
        job_id = str(uuid.uuid4())[:8]
